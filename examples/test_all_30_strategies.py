...
def main():
    ...
    strategy_params = {
        ...
        'heikin_ashi': {},
        'trix': {'period': 15, 'signal_period': 9},
        'aroon': {'period': 14},
    ...
    for i, strategy_name in enumerate(all_strategies, 1):
        ...
        print(f"\n{i:2d}. 测试 {strategy_name:<20} ({cn_name})")
    ...
    if results:
        ...
        print("\n🏆 收益排名 TOP 10:")
        print("-" * 100)
        print(f"{'排名':<4} {'策略':<20} {'中文名':<12} {'收益':<10} {'回撤':<8} {'胜率':<8} {'信号数':<6}")
        print("-" * 100)
        
        for i, r in enumerate(results[:10], 1):
            medal = "🥇" if i == 1 else "🥈" if i == 2 else "🥉" if i == 3 else "  "
            print(f"{medal} {i:<2} {r['name']:<18} {r['cn_name']:<10} {r['return_pct']:>+7.2f}% {r['max_drawdown']:>6.2f}% {r['win_rate']:>6.1f}% {r['total_signals']:>4}")
